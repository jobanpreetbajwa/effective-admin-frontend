...
import BulkProductTabelBody from './bulkProductTabelBody'
...
export default function AddBulkProduct() {
  ...
  const addProductsHandler = async () => {
    try {
      ...
      const newData = Promise.all(promises).then((data) =>
        data.filter((element) => !element?.error)
      )
      ...
      toast.promise(promise, {
        loading: 'Loading...',
        success: () => {
          return ` ${uploadData?.length} products added successfully`
        },
        error: (err) => err || 'Error',
        finally: () => {
  ...
  //handleFileChange function processes an uploaded Excel file, extracts and validates data, then converts it into a structured format compatible with the application's requirements, displaying a preview if successful
  const handleFileChange = (event) => {
    ...
    reader.onload = (e) => {
      ...
      const requiredColumns = ['Product Name', 'Price']
      const headerRow = parsedData[0]

      const isValidFile = requiredColumns.every((column) =>
        headerRow.includes(column)
      )

      if (!isValidFile) {
        alert('The file must contain columns: Product Name,  and Price.')
        return
      }
